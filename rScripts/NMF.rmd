...
subtypes <- factor(subtypes, levels = c(1,2,3))
...
WGCNA clusters genes into network modules using a topological overlap measure (TOM). The TOM is a highly robust measure of network interconnectedness and essentially provides a measure of the connection strength between two adjacent genes and all other genes in a network. Genes are clustered using 1-TOM as the distance measure and gene modules are defined as branches of the resulting cluster tree using a dynamic branch-cutting algorithm
...
## correlation heatmap
svg("../results/nmf/MEs.svg", width = 7, height = 10)
...
for (trait in colnames(datTraits)) {
  ...
  for (module in selectModules) {
    ...
    ## plot
     ...
     rownames(datPlot) <- rownames(datKME[restModule,])
     ## Annotate metbolites id
     rownames(datPlot) <- gsub("^X", "", rownames(datPlot))
     ## match id 
      matchColumnNames <-
       match(rownames(datPlot), metaboliteNames$ID, nomatch = 0)
     ## anotate names
     datPlot$metaboliteName <-
  as.character(metaboliteNames$METABOLITE_NAME)[matchColumnNames]
     ## formula
     formula <- y ~ x  
     ## number of metabolites per modules
     banner("number of metabolites")
     print(paste0("number of metabolites in ", colorOfColumn[column], "-module ", nrow(datPlot)))
     ## plot
    ...
    geom_text_repel(data = head(datPlot[with(datPlot, order(-X, -Y)),], 3),
...
# Heatmap
```{r heatmap, fig.width=13,fig.height=13}
##----------------------------------------------------------------
...
```{r heatmap metPrograme, fig.width=7,fig.height=7}
...
for (program in seq_along(uniquePrograms)) {
  ...
  ## plot
  svg(paste0("../results/nmf/heatmap_", programName, ".svg"), width = 7, height = 7)
  heatmap.p <- pheatmap(
...
```{r heatmap metPrograme print, fig.width=7,fig.height=7}
uniquePrograms <- unique(dataOutput$moduleColors)
...
```{r ext features}
## extract features
metasetTop <- extractFeatures(resModel, 30)
## metabolites names
...
for (i in 1:length(metasetTop)) {
  name <- paste0("subtype", i)
  topMet <- metabolites[metasetTop[[i]]]
  df <- as.data.frame(topMet)
  colnames(df) <- name
  topMetdf <- as.data.frame(append(topMetdf, df))
  ## panel metabolites
  panel <- gsub("^X", "", topMet)
  ## match
...
write.csv(topMetdf, "../results/nmf/topMetabolitesperSubtype.csv",
         row.names = FALSE)
```
...
for (i in seq_along(groups)) {
  ...
  p <-
    ...
    labs(fill = "Metabololites category",
         ...
         y = "p value (-log10)")  +
    # scale_x_continuous(trans = "log10", limits = c(0.5,1.5)) +
    theme(legend.position = "none") +
