---
title: "transcriptomics data SMPD1"
...
## go terms associated with Masenchymal stem cell
go_id <- GOID(GOTERM[Term(GOTERM) %like% "sphingolipid" 
                       ])

go2genes <- function(x) {
 for (i in seq_along(x)) {
   allegs <- try(get(x[[i]], org.Hs.egGO2ALLEGS), TRUE)
   if(isTRUE(class(allegs)=="try-error")) { next } else { 
      x[i]= allegs 
     }
   symbol <- unlist(mget(allegs, org.Hs.egSYMBOL))
   out <- symbol
   genes <- unique(out)
 }
 return(genes)
...
f1 <- kOverA(5, 10)
flist <- filterfun(f1)
ans <- genefilter(tcgaSubset, flist)

tcgaSubset <- tcgaSubset[rownames(tcgaSubset) %in% names(ans[ans==TRUE]),]

## save data
write.csv(tcgaDESeq, "../results/tcgaDESeq_sphingolipids.csv")

ssgseaSubset <- tcgaSubset
...
svg("../results/nmf/heatmap_TCGA_sphingolipids.svg", width = 25, height = 25)
